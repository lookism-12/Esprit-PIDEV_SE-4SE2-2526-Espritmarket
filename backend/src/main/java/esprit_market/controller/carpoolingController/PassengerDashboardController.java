...
public class PassengerDashboardController {
    ...
    public PassengerDashboardDTO getDashboard(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        List<BookingResponseDTO> bookings = bookingRepository
                .findByPassengerProfileId(profile.getId()).stream()
                .map(bookingMapper::toResponseDTO)
                .collect(Collectors.toList());

        List<RideRequestResponseDTO> requests = rideRequestRepository
                .findByPassengerProfileId(profile.getId()).stream()
        ...
        long pending = requests.stream()
                .filter(r -> r.getStatus() == RideRequestStatus.PENDING).count();
    ...
    private PassengerProfile getOrCreateProfile(User user) {
        ...
        var existing = passengerProfileRepository.findByUserId(user.getId());
        ...
        log.info("Passenger profile created with id: {}", saved.getId());

        // Update user with profile id and PASSENGER role
        try {
            User freshUser = userRepository.findById(user.getId()).orElse(user);
            freshUser.setPassengerProfileId(saved.getId());

            List<Role> roles = freshUser.getRoles() != null
                    ? new ArrayList<>(freshUser.getRoles())
                    : new ArrayList<>();
            ...
            // Profile was created — user update failure is non-fatal
            log.warn("Could not update user roles after profile creation: {}", e.getMessage());
        }
