...
public class CartServiceImpl implements ICartService {
     ...
     */
    private Cart createNewCart(ObjectId userId) {
        ...
        // Attach cart ID to user profile
        User user = userRepository.findById(userId).orElse(null);
        if (user != null) {
            user.setCartId(saved.getId());
    ...
    public CartResponse checkout(ObjectId userId, CheckoutRequest request) {
        ...
        Cart cart = cartRepository.findByUserIdAndStatus(userId, CartStatus.DRAFT)
                .orElseThrow(() -> new CartNotFoundException("No cart found to checkout"));
