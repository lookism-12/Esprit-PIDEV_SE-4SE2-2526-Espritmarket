...
export default class ${entityName} {
  constructor() {}
    ...
    if (scriptContent) {
        ...
        return processAnalogScriptV2(filePath, project.createSourceFile(filePath, scriptContent), project.createSourceFile(`${filePath}.virtual.ts`, source), ngType, entityName, shouldFormat);
    }
    return source;
}
function processAnalogScriptV2(fileName, ngSourceFile, targetSourceFile, ngType, entityName, isProd) {
    ...
    const classMembers = new Map(), callsQueue = [];
        ...
        if (Node.isVariableStatement(node)) {
            ...
            if (Node.isArrayBindingPattern(nameNode) ||
                ...
                for (const bindingElement of bindingElements) {
                    ...
                    targetConstructor.addStatements(`this.${bindingElement} = ${bindingElement};`);
                }
                continue;
            }
    ...
    for (const classMember of classMembers.values()) {
        const { isLet, hasExportKeyword, name, initializer, declaration } = classMember;
        ...
        if (isLet) {
            ...
            gettersSetters.push({
                propertyName: name,
                isFunction: !!initializer && Node.isFunctionLikeDeclaration(initializer),
            });
        }
        else {
            targetClass.addProperty({
...
function processInitializer(writer, declaration, classMembers, initializer) {
    if (!initializer) {
        console.warn(`[Analog] const variable must have an initializer: ${declaration.getName()}`);
        return;
    }
