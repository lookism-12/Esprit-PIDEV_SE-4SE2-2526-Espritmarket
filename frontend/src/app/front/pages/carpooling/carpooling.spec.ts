...
const mockRide: RideResponseDTO = {
  rideId: 'r1', driverProfileId: 'dp1', driverName: 'John',
  vehicleId: 'v1', departureLocation: 'Tunis', destinationLocation: 'Sfax',
  departureTime: '2026-04-01T10:00:00', availableSeats: 3,
  pricePerSeat: 5, status: 'ACCEPTED',
};
...
function makeAuthSvc(role: UserRole = UserRole.PASSENGER): AuthService {
  return {
    ...
    userId: signal('user1'),
    currentUser: signal({ id: 'user1', role }),
...
describe('Carpooling Component — Passenger', () => {
  ...
  it('activeView defaults to passenger', () => {
    expect(component.activeView()).toBe('passenger');
  });

  it('setView switches to driver', () => {
    component.setView('driver');
    expect(component.activeView()).toBe('driver');
  });

  it('passengerTab defaults to rides', () => {
    expect(component.passengerTab()).toBe('rides');
  });

  it('isDriverUser returns false for PASSENGER role', () => {
    expect(component.isDriverUser()).toBe(false);
  });

  it('filteredRides: only shows CONFIRMED rides', () => {
    component.rides.set([
      { ...mockRide, status: 'CONFIRMED' },
      { ...mockRide, rideId: 'r2', status: 'CANCELLED' },
    ]);
  ...
  it('filteredRides: sorts by cheapest', () => {
    component.rides.set([
      { ...mockRide, rideId: 'r1', pricePerSeat: 10, status: 'CONFIRMED' },
      { ...mockRide, rideId: 'r2', pricePerSeat: 3, status: 'CONFIRMED' },
    ]);
  ...
  it('filteredRides: sorts by fastest', () => {
    component.rides.set([
      { ...mockRide, rideId: 'r1', estimatedDurationMinutes: 90, status: 'CONFIRMED' },
      { ...mockRide, rideId: 'r2', estimatedDurationMinutes: 30, status: 'CONFIRMED' },
    ]);
    component.setSort('fastest');
    expect(component.filteredRides()[0].estimatedDurationMinutes).toBe(30);
  });

  it('isRideFull: true when availableSeats is 0', () => {
    expect(component.isRideFull({ ...mockRide, availableSeats: 0 })).toBe(true);
  });

  it('isRideFull: false when seats available', () => {
    expect(component.isRideFull({ ...mockRide, availableSeats: 2 })).toBe(false);
  });
  ...
  it('starsArray: returns 5 booleans', () => {
    expect(component.starsArray(3)).toHaveLength(5);
  });

  it('starsArray: first 3 true for rating 3', () => {
    expect(component.starsArray(3).filter(Boolean)).toHaveLength(3);
  });

  it('starsArray: all false for rating 0', () => {
    expect(component.starsArray(0).filter(Boolean)).toHaveLength(0);
  });

  it('starsArray: all true for rating 5', () => {
    expect(component.starsArray(5).filter(Boolean)).toHaveLength(5);
  });
  ...
  it('closeBookingPanel: sets to false', () => {
    component.showBookingPanel.set('r1');
    component.closeBookingPanel();
    expect(component.showBookingPanel()).toBe(false);
  });

  it('openCreateRideRequestModal: sets flag to true', () => {
    component.openCreateRideRequestModal();
    expect(component.showCreateRideRequestModal()).toBe(true);
  });

  it('closeCreateRideRequestModal: sets flag to false', () => {
    component.showCreateRideRequestModal.set(true);
    component.closeCreateRideRequestModal();
  ...
  it('submitRideRequest: calls service and closes modal on success', () => {
    component.createRideRequestForm.patchValue({
      departureLocation: 'Tunis', destinationLocation: 'Sfax',
      departureTime: '2026-04-01T09:00', requestedSeats: 1, proposedPrice: 5,
    });
    component.showCreateRideRequestModal.set(true);
    component.submitRideRequest();
  ...
  it('submitRideRequest: sets error on failure', () => {
    (svc.createRideRequest as ReturnType<typeof vi.fn>).mockReturnValue(
  ...
  it('joinCoRequest: shows error when PENDING', () => {
    component.joinCoRequest({ ...mockRequest, status: 'PENDING' });
    expect(component.error()).toContain('No driver');
  });

  it('joinCoRequest: shows error when CANCELLED', () => {
    component.joinCoRequest({ ...mockRequest, status: 'CANCELLED' });
    expect(component.error()).toContain('cancelled');
  });

  it('joinCoRequest: opens booking panel when ACCEPTED with rideId', () => {
    component.hasPassengerProfile.set(true);
    component.joinCoRequest({ ...mockRequest, status: 'ACCEPTED', rideId: 'r1' });
    expect(component.showBookingPanel()).toBe('r1');
  });

  it('joinCoRequest: shows error when ACCEPTED but no rideId', () => {
    component.joinCoRequest({ ...mockRequest, status: 'ACCEPTED', rideId: undefined });
    expect(component.error()).toContain('not available');
  });

  it('formatPrice: formats with TND suffix', () => {
    expect(component.formatPrice(5.5)).toBe('5.5 TND');
  });

  it('formatDate: returns formatted date string', () => {
    expect(component.formatDate('2026-04-01T10:00:00')).toBeTruthy();
  });
});
...
describe('Carpooling Component — Driver', () => {
  ...
  it('calls getDriverProfile for driver user', () => {
    expect(svc.getDriverProfile).toHaveBeenCalled();
  });

  it('isDriverUser returns true for DRIVER role', () => {
    expect(component.isDriverUser()).toBe(true);
  });

  it('switches to driver view when ?view=driver query param', () => {
    expect(component.activeView()).toBe('driver');
  });

  it('hasDriverProfile returns true when profile loaded', () => {
    expect(component.hasDriverProfile()).toBe(true);
  });

  it('driverProfile is set after load', () => {
    expect(component.driverProfile()).not.toBeNull();
  });
});
