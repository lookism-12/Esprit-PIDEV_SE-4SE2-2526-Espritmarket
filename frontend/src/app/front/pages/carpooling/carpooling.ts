...
import { CarpoolingService, RideResponseDTO, VehicleDTO, DriverProfileDTO, BookingResponseDTO, RideRequestResponseDTO, DriverStatsDTO } from '../../core/carpooling.service';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CurrencyPipe, DatePipe, ReactiveFormsModule, RouterModule],
...
export class Carpooling implements OnInit {
  ...
  filteredRides = computed(() => {
    ...
    const list = [...this.rides()].filter(r => r.status === 'CONFIRMED');
  ...
  ngOnInit(): void {
    ...
    // Only load driver data if the user is actually a driver
    ...
    this.route.queryParams.subscribe(params => {
      if (params['view'] === 'driver') {
        this.activeView.set('driver');
      }
      // Auto-open ride request modal from passenger dashboard
  ...
  private loadPassengerView(): void {
    this.carpoolingService.getAllRides().subscribe({
  ...
  openBookingPanel(rideId: string): void {
    if (!this.hasPassengerProfile()) {
      this.carpoolingService.registerAsPassenger().subscribe({
        ...
        error: () => this.error.set('Could not activate passenger profile. Please try again.')
      });
      return;
    }
  ...
  confirmBooking(ride: RideResponseDTO): void {
    ...
    this.carpoolingService.createBooking({
      rideId: ride.rideId,
      pickupLocation,
      dropoffLocation,
      numberOfSeats: 1
  ...
  submitRideRequest(): void {
    ...
    this.carpoolingService.createRideRequest({
      ...v,
  ...
  private loadDriverView(): void {
    this.carpoolingService.getDriverProfile().subscribe({
      ...
      error: (err) => {
        console.error('getDriverProfile failed', err);
        this.driverProfile.set(null);

        const userId = this.auth.userId();
        if (this.isDriverUser() && userId) {
          // Fallback: some tokens/users fail /me resolution; try by userId
          this.carpoolingService.getDriverProfileByUserId(userId).subscribe({
            ...
            error: (e2) => console.error('getDriverProfileByUserId failed', e2)
          });
        }
      }
    });
  ...
  private loadDriverStats(): void {
    ...
    this.carpoolingService.getMyDriverStats().subscribe({
      next: s => this.driverStats.set(s),
      error: e => console.error('getMyDriverStats failed', e)
    });
  }
      ...
      next: rides => {
        ...
        rides.forEach(r => {
          this.carpoolingService.getBookingsForRide(r.rideId).subscribe({
            next: bookings => {
              this.myBookings.update(m => { m.set(r.rideId, bookings); return new Map(m); });
            },
            error: () => { }
          });
        });
  ...
  getBookingsForRide(rideId: string): BookingResponseDTO[] {
  ...
  registerAsDriver(): void {
    ...
    this.carpoolingService.registerAsDriver({ licenseNumber, licenseDocument }).subscribe({
      next: p => {
        this.success.set('✓ Driver profile created! You can now accept passenger requests and post rides.');
      ...
      error: e => {
        console.error('registerAsDriver failed', e);
        const backendMessage =
          e?.error?.message ??
          e?.error ??
          e?.statusText ??
          e?.message;
        this.error.set(backendMessage || 'Registration failed');
      }
    });
  }
  ...
  submitVehicle(): void {
    ...
    this.carpoolingService.addVehicle(this.addVehicleForm.value).subscribe({
      next: v => {
        this.success.set('✓ Vehicle added successfully! You can now accept requests with this vehicle.');
        setTimeout(() => this.success.set(null), 4000);
  ...
  createRide(): void {
    ...
    this.carpoolingService.createRide({
      ...
      next: () => {
        this.success.set('✓ Your ride has been published! Passengers can now see it.');
        setTimeout(() => this.success.set(null), 4000);
  ...
  submitAcceptRequest(): void {
    ...
    this.carpoolingService.acceptRideRequest(this.selectedRequestId()!, vehicleId).subscribe({
      next: () => {
        this.success.set('✓ Request accepted! You are now in charge of this ride. The passenger has been notified.');
        setTimeout(() => this.success.set(null), 4000);
  ...
  quickAcceptRequest(req: RideRequestResponseDTO): void {
    ...
    if (!vehicleId) {
      this.error.set('Add a vehicle first to quick-accept requests.');
      this.openAddVehicleModal();
      return;
    }
    this.carpoolingService.acceptRideRequest(req.id, vehicleId).subscribe({
      next: () => {
        this.success.set('✓ Request accepted with quick accept! The passenger has been notified.');
        setTimeout(() => this.success.set(null), 4000);
        this.carpoolingService.getAvailableRideRequests().subscribe();
        this.loadMyRidesAndVehicles();
      },
      error: e => this.error.set(e?.error?.message || 'Could not quick-accept request')
    });
  }
  ...
  joinCoRequest(req: RideRequestResponseDTO): void {
    if (req.status === 'PENDING') {
      this.error.set('No driver has accepted this request yet — check back later.');
      return;
    }
    if (req.status === 'CANCELLED') {
      this.error.set('This request has been cancelled.');
      return;
    }
    if (req.status === 'ACCEPTED' && req.rideId) {
      this.openBookingPanel(req.rideId);
    } else {
      this.error.set('Ride details are not available yet.');
    }
  }
  ...
  submitCounterPrice(): void {
    ...
    this.carpoolingService.counterPrice(this.selectedRequest()!.id, price, note).subscribe({
      next: () => {
        this.success.set('✓ Counter offer sent! The passenger will see your offer.');
        setTimeout(() => this.success.set(null), 4000);
