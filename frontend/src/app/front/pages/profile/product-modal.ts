    ...
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            <h2 class="text-2xl font-black text-dark tracking-tight">{{ mode() === 'add' ? 'List New Product' : 'Edit Product' }}</h2>
            <p class="text-sm text-secondary font-medium mt-1">Fill in the details for your marketplace listing</p>
          </div>
          <button (click)="onClose()" class="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-gray-200 text-gray-400 hover:text-dark transition-all text-2xl">&times;</button>
        </div>
