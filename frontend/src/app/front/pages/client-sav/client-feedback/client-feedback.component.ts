...
export class ClientFeedbackComponent {
  ...
  private loadMyCartItems(): void {
    ...
    // On récupère désormais les articles de TOUTES les commandes passées, pas seulement le panier DRAFT
  ...
  private loadMyFeedbacks(items: CartItemOption[]): void {
    ...
    forkJoin(calls)
      .pipe(finalize(() => this.isLoadingMyFeedbacks.set(false)))
      .subscribe({
        next: (groups) => {
  ...
  removePending(feedback: SavFeedback): void {
    ...
    const ok = confirm('Supprimer cet avis non traite ?');
    if (!ok) {
      return;
    }
    ...
    this.savService.deleteFeedback(feedback.id).subscribe({
      next: () => {
        ...
        if (this.hasSearched() && this.searchItemId().trim()) {
          this.trackFeedback();
        }
      },
      error: () => {
        this.errorMsg.set('Suppression impossible pour le moment.');
      }
    });
  }
  ...
  setPriority(priority: FeedbackPriority): void {
    this.feedbackForm.patchValue({ priority });
  }
  ...
  setRecommend(value: boolean): void {
    this.feedbackForm.patchValue({ recommendsProduct: value });
  }
  ...
  submitFeedback() {
    ...
    save$.subscribe({
      next: () => {
        ...
        this.successMsg.set(this.editingFeedback()
          ? 'Votre dossier a ete modifie.'
  ...
  private buildPayload(): SavFeedbackRequest {
    ...
    const isComplaint = formValue.type === 'SAV';
    const existing = this.editingFeedback();

    return {
      type: formValue.type,
      cartItemId: formValue.cartItemId,
      reason: formValue.reason || '',
      message: formValue.message,
      rating: isComplaint ? 3 : Number(formValue.rating || 5),
      status: existing?.status || 'PENDING',
      problemNature: isComplaint ? formValue.problemNature : undefined,
      priority: isComplaint ? formValue.priority : undefined,
      desiredSolution: isComplaint ? formValue.desiredSolution : undefined,
      positiveTags: isComplaint ? [] : (formValue.positiveTags || []),
      recommendsProduct: isComplaint ? undefined : !!formValue.recommendsProduct,
  ...
  // --- UI Helpers ---
  getStatusInfo(status: FeedbackStatus) {
    return this.statusMap[status];
  }

  isComplaintType(): boolean {
    return this.feedbackForm.get('type')?.value === 'SAV';
  }

  selectedTags(): string[] {
    return (this.feedbackForm.get('positiveTags')?.value as string[]) || [];
  }

  messageLimit(): number {
    return this.isComplaintType() ? 500 : 300;
  }
  ...
  messageRemaining(): number {
    return Math.max(0, this.messageLimit() - this.messageLength());
  }

  canEdit(feedback: SavFeedback): boolean {
