...
describe('CarpoolingService', () => {
  ...
  it('createBooking: isLoading resets to false after emit', () => {
    ...
    service.createBooking({ rideId: 'r1', numberOfSeats: 1, pickupLocation: 'A', dropoffLocation: 'B' }).subscribe(() => { loadingAfter = service.isLoading(); });
  ...
  it('acceptRideRequest: calls POST with vehicleId in body', () => {
    ...
    expect(http.post).toHaveBeenCalledWith('/api/ride-requests/req1/accept', { vehicleId: 'v1' });
  });
