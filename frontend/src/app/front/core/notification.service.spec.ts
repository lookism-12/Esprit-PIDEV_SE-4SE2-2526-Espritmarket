...
describe('NotificationService', () => {
  ...
  describe('getMy()', () => {
    ...
    it('returns list of notifications', () => {
      const list = [makeNotifResponse(), makeNotifResponse({ id: 'notif-002' })];
      service.getMy().subscribe(r => expect(r).toHaveLength(2));

      http.expectOne(`${BASE}/my`).flush(list);
    });
  });

  // ── getNotifications (wrapped) ──────────────────────────────────────────────
  describe('getNotifications()', () => {
    it('wraps getMy() result in { notifications }', () => {
      const list = [makeNotifResponse()];
      service.getNotifications().subscribe(r => {
    ...
    it('maps backend description field to message', () => {
      ...
      service.getNotifications().subscribe(r => {
        expect(r.notifications[0].message).toBe('Backend description');
      });
    ...
    it('maps notification_status to active', () => {
      ...
      service.getNotifications().subscribe(r => {
        expect(r.notifications[0].active).toBe(false);
      });
  ...
  describe('getAllAdmin()', () => {
    ...
    it('returns mapped AppNotification array', () => {
      const list = [makeNotifResponse(), makeNotifResponse({ id: 'n2', title: 'Second' })];
  ...
  describe('markAsRead()', () => {
    ...
    it('returns updated notification with read=true', () => {
      service.markAsRead('notif-001').subscribe(r => expect(r.read).toBe(true));

      http.expectOne(`${BASE}/notif-001/read`).flush(makeNotifResponse({ read: true }));
    });
  });
  ...
  describe('markAllAsRead()', () => {
    it('marks all unread notifications as read', () => {
      ...
      // Should PATCH only the 2 unread ones
    ...
    it('returns success immediately when all notifications are already read', () => {
      const alreadyRead = makeNotifResponse({ id: 'n1', read: true });

      service.markAllAsRead().subscribe(r => expect(r.success).toBe(true));

      http.expectOne(`${BASE}/my`).flush([alreadyRead]);
      // No PATCH requests should be made
  ...
  // ── toAppNotifications (private — tested via getNotifications) ──────────────
  describe('toAppNotifications mapping', () => {
    it('falls back to empty string when both description and message are missing', () => {
      ...
      service.getNotifications().subscribe(r => {
        expect(r.notifications[0].message).toBe('');
      });
    ...
    it('prefers description over message when both present', () => {
      ...
      service.getNotifications().subscribe(r => {
        expect(r.notifications[0].message).toBe('from backend');
      });
